...
export default function Home() {
  ...
  return (
    <div className={`flex flex-col items-center justify-start p-8 md:p-32 h-screen overflow-auto ${isLoaded ? 'no-blur' : 'blur-effect'}`}>
      <motion.div
      ...
      >
        ...
        >
          <Link href="/design" legacyBehavior>
            <a className='flex w-full'>
              <div className='flex flex-col items-start transition-opacity duration-150 hover:bg-neutral-100 rounded-lg p-1.5 w-full cursor-pointer'>
        ...
        >
          ...
          <div className="space-y-2">
            <a href="https://cambrean.com" target="_blank" rel="noopener noreferrer" className="flex items-center cursor-pointer space-x-4 mt-4 transition-colors duration-150 hover:bg-neutral-100 p-2.5 rounded-xl">
          ...
          <div className='space-y-2'>
            {recognitionLinks.map((link, index) => (
              <motion.a
                ...
                className='block p-2 rounded-lg hover:bg-neutral-100 transition-colors duration-150 cursor-pointer'
                initial={{ opacity: 0, y: 40 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.8 + index * 0.2, duration: 0.8 }}
              >
                <div className='cursor-pointer'>
                  <span className='text-sm text-neutral-900 block'>{link.text}</span>
                  <p className='text-xs text-neutral-500 mt-1'>{link.subtext}</p>
                </div>
              </motion.a>
            ))}
          </div>
        ...
        >
          ...
          <div className='flex space-x-4 pt-4'>
            <a href="mailto:me@davidsloane.xyz" className="inline-block text-sm cursor-pointer transition-colors duration-150 hover:bg-stone-100 p-2 rounded-md text-muted-foreground">email</a>
            <a href="https://github.com/dtsloane" className="inline-block text-sm cursor-pointer transition-colors duration-150 hover:bg-stone-100 p-2 rounded-md text-muted-foreground">Github</a>
            <a href="https://x.com/sloanerdog" className="inline-block text-sm cursor-pointer transition-colors duration-150 hover:bg-stone-100 p-2 rounded-md text-muted-foreground">X</a>
            <a href="https://www.linkedin.com/in/dtsloane/" className="inline-block text-sm cursor-pointer transition-colors duration-150 hover:bg-stone-100 p-2 rounded-md text-muted-foreground">LinkedIn</a>
          </div>
        </motion.section>
      </motion.div>
    </div>
